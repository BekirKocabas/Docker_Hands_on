...
- Run a `alpine` container with interactive shell open, name the container as `bekir`, attach the volume `cw-vol` to `/cw` mount point in the container, and add command to run alpine shell. Here, explain `--volume` and `v` flags.

```bash
docker run -it --name bekir -v cw-vol:/cw alpine ash
```

- List files/folder in `bekir` container, show mounting point `/cw`, and explain the mounted volume `cw-vol`.

```bash
ls
```

- Create a file in `bekir` container under `/cw` folder.

```bash
cd cw && echo "This file is created in the container bekir" > i-will-persist.txt
```
...
- Exit the `bekir` container and return to ec2-user bash shell.
...
- Remove the `bekir` container.

```bash
docker rm bekir
```

- Show the list of all containers, and the `bekir` container is gone.
...
- Run a `alpine` container with interactive shell open, name the container as `bekir2nd`, attach the volume `cw-vol` to `/cw2nd` mount point in the container, and add command to run alpine shell.

```bash
docker run -it --name bekir2nd -v cw-vol:/cw2nd alpine ash
```
...
- Create an another file in `bekir2nd` container under `/cw2nd` folder.

```bash
cd cw2nd && echo "This is a file of the container bekir2nd" > loadmore.txt
```
...
- Exit the `bekir2nd` container and return to ec2-user bash shell.

```bash
exit
```

- Run a `ubuntu` container with interactive shell open, name the container as `bekir3rd`, attach the volume `cw-vol` to `/cw3rd` mount point in the container, and add command to run bash shell.

```bash
docker run -it --name bekir3rd -v cw-vol:/cw3rd ubuntu bash
```
...
- Create an another file in `bekir3rd` container under `/cw3rd` folder.

```bash
cd cw3rd && touch file-from-3rd.txt && ls
```

- Exit the `bekir3rd` container and return to ec2-user bash shell.

```bash
exit
```

- Run an another `ubuntu` container with interactive shell open, name the container as `bekir4th`, attach the volume `cw-vol` as read-only to `/cw4th` mount point in the container, and add command to run bash shell.

```bash
docker run -it --name bekir4th -v cw-vol:/cw4th:ro ubuntu bash
```
...
- Exit the `bekir4th` container and return to ec2-user bash shell.
...
- Delete `bekir2nd`, `bekir3rd` and `bekir4th` containers.

```bash
docker rm bekir2nd bekir3rd bekir4th
```
...
- Run the `clarusway/hello-clarus` container with interactive shell open, name the container as `bekir`, and show the inside of `hello-bekir` directory.

```bash
docker run -it --name bekir clarusway/hello-clarus sh
/ # ls
