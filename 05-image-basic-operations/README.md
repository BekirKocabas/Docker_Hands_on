...
```bash
docker build -t "bekirkocabas/flask-app:1.0" .
docker image ls
```
...
```bash
docker run -d --name welcome -p 80:80 bekirkocabas/flask-app:1.0
docker container ls
```
...
```bash
docker push bekirkocabas/flask-app:1.0
```
...
```bash
docker build -t "bekirkocabas/flask-app:2.0" -f ./Dockerfile-alpine . 
docker image ls
```

- Note that while the size of `bekirkocabas/flask-app:1.0` is approximately 400MB, the size of `bekirkocabas/flask-app:2.0` is 56MB.

- Run the newly built image as container in detached mode, connect host `port 80` to container `port 80`, and name container as `welcome`. Then list running containers and connect to EC2 instance from the browser to show the Flask app is running.

```bash
docker run -d --name welcome -p 8080:80 bekirkocabas/flask-app:2.0
docker ps
```
...
```bash
docker push bekirkocabas/flask-app:2.0
```

- We can also tag the same image with different tags.

```bash
docker image tag bekirkocabas/flask-app:2.0 bekirkocabas/flask-app:latest
```
